...
class Frame(object):
    ...
    def GetPuzzleCircles(self):
        xdim = len(self.PixelMap)
        ydim = len(self.PixelMap[0])
        for x in range(xdim):
            # Search black parts in each line
            Bp = np.where(self.PixelMap[x] == (0,0,0))[0]
            if len(Bp) > 0:
                #Find blocks of the black parts
                    BpSplit = np.split(Bp,  np.where(np.diff(Bp)!=1)[0])
                    for Bb in BpSplit:
                        y = np.floor(np.mean(Bb)).astype(np.int)
                        if (self.NotIn([x,y])):
